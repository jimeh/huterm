...
use huterm_protocol::{
    ...
    GridSize, Rgb, ScrollCommand, TerminalId, TerminalModes, TerminalRow,
...
impl TerminalEngine {
    ...
    pub(crate) fn scroll(&mut self, command: ScrollCommand) {
        let scroll = match command {
            ScrollCommand::Relative(rows) => {
                Scroll::Delta(i32::try_from(rows).unwrap_or(if rows < 0 {
                    i32::MIN
                } else {
                    i32::MAX
                }))
            ...
            ScrollCommand::Absolute(offset) => {
                ...
                let delta = offset.min(self.term.history_size()) as i128
                    - current as i128;
    ...
    pub(super) fn viewport_state(&self) -> (usize, usize) {
        (self.term.grid().display_offset(), self.term.history_size())
    }

    pub(crate) fn snapshot(&mut self) -> TerminalSnapshot {
        let columns = self.term.columns();
        let rows = self.term.screen_lines();
        let history_size = self.term.history_size();
        let bottom_offset = self.term.grid().display_offset();
        let top_line = -i32::try_from(bottom_offset).unwrap_or(i32::MAX);
        let full = self.retained_rows.len() != rows
            || self
                .retained_rows
                .first()
        ...
        for (row, damaged) in dirty.into_iter().enumerate() {
            if !damaged {
                continue;
            }
            ...
            let row_value = Arc::new(TerminalRow { cells });
            if row < self.retained_rows.len() {
                self.retained_rows[row] = row_value;
            } else {
                self.retained_rows.push(row_value);
            }
        }
        self.retained_rows.truncate(rows);
    ...
    pub(super) fn drain_effects(&self) -> Vec<EngineEffect> {
        self.events
            .try_iter()
...
mod tests {
    ...
    #[test]
    fn sparse_damage_reuses_clean_rows_and_keeps_old_snapshots_immutable() {
        let mut engine = engine();
        engine.process(b"one\r\ntwo\r\nthree");
        let old = engine.snapshot();
        engine.process(b"\x1b[2;1HX");
        let skipped = engine.snapshot();
        engine.process(b"\x1b[2;2HY");
        let latest = engine.snapshot();
        assert!(Arc::ptr_eq(&old.rows[0], &latest.rows[0]));
        assert!(!Arc::ptr_eq(&old.rows[1], &latest.rows[1]));
        assert_eq!(old.rows[1].cells[0].text, "t");
    ...
    #[test]
    fn metadata_and_dynamic_colors_refresh_with_retained_rows() {
        let mut engine = engine();
        engine.process(b"A");
        let before = engine.snapshot();
        engine.process(b"\x1b[?2004h\x1b[?25l");
        let modes = engine.snapshot();
        assert!(modes.modes.bracketed_paste);
        assert_eq!(modes.cursor.unwrap().shape, CursorShape::Hidden);
        assert!(Arc::ptr_eq(&before.rows[1], &modes.rows[1]));
        engine.process(b"\x1b]10;#123456\x07");
        let colored = engine.snapshot();
        assert_eq!(
            colored.rows[0].cells[0].foreground,
            CellColor::Rgb(Rgb {
                red: 0x12,
                green: 0x34,
                blue: 0x56
        ...
        assert_eq!(
            before.rows[0].cells[0].foreground,
            CellColor::DefaultForeground
        );
    }

    #[test]
    fn shared_viewport_anchors_output_and_resize_without_client_compensation() {
        let mut engine = engine();
        engine.process(b"0\r\n1\r\n2\r\n3\r\n4\r\n5");
        engine.scroll(ScrollCommand::Absolute(2));
        let before = engine.snapshot();
        engine.process(b"\r\n6");
        let output = engine.snapshot();
        assert_eq!(before.rows[0], output.rows[0]);
        for rows in [2, 5, 3] {
            engine.resize(GridSize::clamped(8, rows));
            assert_eq!(engine.snapshot().rows[0], before.rows[0]);
        }
        engine.scroll(ScrollCommand::Live);
        assert_eq!(engine.snapshot().viewport.bottom_offset, 0);
    }
    ...
    fn snapshot_should_parse_color_style_and_wide_cells() {
        ...
        let snapshot = engine.snapshot();
        let first = &snapshot.rows[0].cells[0];
        let wide = &snapshot.rows[0].cells[1];
        let spacer = &snapshot.rows[0].cells[2];
    ...
    #[test]
    fn shared_scroll_reads_history_without_changing_content_generation() {
        let mut engine = engine();
        engine.process(b"one\r\ntwo\r\nthree\r\nfour");

        let live = engine.snapshot();
        engine.scroll(ScrollCommand::Absolute(1));
        let scrolled = engine.snapshot();
        engine.scroll(ScrollCommand::Live);
        let live_again = engine.snapshot();

        assert!(live.history_size > 0);
        assert_ne!(scrolled.rows, live.rows);
        assert_eq!(live_again, live);
        assert!(live.cursor.is_some());
        assert!(scrolled.cursor.is_none());

        engine.scroll(ScrollCommand::Absolute(usize::MAX));
        let clamped = engine.snapshot();
