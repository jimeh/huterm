...
- `huterm-procinfo` reports operating-system process facts: Linux through
  ...
  silence, or a title change, plus a one-second poll while a job holds the
  foreground. Idle shells arm no deadline, and the output path never calls
  `tcgetpgrp`. Never probe at spawn: a child that has not exec'd yet reads as
  Huterm. An OSC 7 directory or OSC 0/2 title report applies only while the
  foreground group that was active when it arrived keeps the foreground;
  otherwise publish the probed foreground directory, falling back to the root's.
  Read the group for a title only when its text changes, its recorded report no
  longer applies, job-control input arrived since the last probe, or a repeated
  title's attribution is 250 ms old.
