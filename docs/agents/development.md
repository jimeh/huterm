...
the window-level capture that also serves tab reordering. It sits inboard of the
sidebar resize handle by the handle's width so the two never overlap, and both
tab bar indicators fade after 700 ms rather than the terminal's two seconds.
Explicit selection or creating a tab reveals it; ordinary redraws preserve
manual scroll. Drag a tab to reorder it within its window, horizontally or
...
terminal. Reordering preserves terminal processes, focus, selection, and scroll
position. Cross-window moves and tear-out remain deferred. Reload applies
placement, font, padding, and theme changes across all windows. Shell titles
label tabs, with the launched program as fallback. Directory and process labels
and directory inheritance are not implemented yet.
